<template>
  <div class="play-bar" :class="{show:!toggle}">
    <div class="item-up" @click="toggle=!toggle" >
      <svg style="top: 3px;" class="icon" :class="{turn: toggle}">
          <use xlink:href="#icon-jiantou-xia-cuxiantiao"></use>
      </svg>
      <span class="a-or-p" v-if="toggle==true">点击隐藏</span>
      <span class="a-or-p" v-else="toggle==false">点击展开</span>
    </div>
    <div class="kongjian">
      <!-- 上一首 -->
      ...
      <div class="playing-speed">
        ...
        <div class="item item-volume">
          ...
          <svg v-else="volume == 0" class="icon">
        ...
        <!-- 当前播放的歌曲列表 -->
        <div id="playAside" class="item" @click="changeAside">
          <svg class="icon">
...
export default {
  ...
  data() {
    return {
      ...
      tag: false, //拖拽开始结束的标志，开始拖拽才为true
      volume: 50, //音量默认一半
      toggle: true,  //显示隐藏播放器
    };
  },
  computed: {
    ...mapGetters([
      ...
      "duration", //音乐时长
      "curTime", //当前音乐的播放位置
      "showAside", //是否显示正在播放的歌曲列表
      'listIndex',  //当前播放的个曲在歌单中的位置
      'listOfSongs',  //当前歌单列表
      'autoNext',  //自动播放下一曲
      'loginIn',  //是否登录
      'userId', //当前登录用户的id
      'isActive',  //当前播放歌曲是否已经收藏
    ]),
  },
  watch: {
    //切换播放状态的图标
  ...
  methods: {
    ...
    mousemove(e) {
      ...
      if (this.tag) {
        let movementX = e.clientX - this.mouseStartX; //点移动的距离
    ...
    next(){
       if(this.listIndex != -1 && this.listOfSongs.length > 1){    //当前处于不可能状态或者只有只有一首音乐的时候不执行）
                if(this.listIndex < this.listOfSongs.length-1){                                 //不是最后一首音乐
                    this.$store.commit('setListIndex',this.listIndex + 1);  //直接返回最后一收
                }else{                                                  //当前是第一首音乐
